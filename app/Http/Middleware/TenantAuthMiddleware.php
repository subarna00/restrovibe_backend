    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (!$token) {
            return response()->json([
                'data' => null,
                'message' => 'Unauthorized access. Tenant authentication required.',
                'status' => 'failed',
                'statusCode' => 401
            ], 401);
        }
        ...
        if (!$personalAccessToken) {
            return response()->json([
                'data' => null,
                'message' => 'Invalid token. Tenant authentication required.',
                'status' => 'failed',
                'statusCode' => 401
            ], 401);
        }
        ...
        if (!$user) {
            return response()->json([
                'data' => null,
                'message' => 'Invalid user. Tenant authentication required.',
                'status' => 'failed',
                'statusCode' => 401
            ], 401);
        }
